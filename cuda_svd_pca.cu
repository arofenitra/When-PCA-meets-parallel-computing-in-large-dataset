...
public:
    CudaSVDPCA(int size) : n(size) {
        ...
        CHECK_CUDA(curandCreateGenerator(&curandGen, CURAND_RNG_PSEUDO_DEFAULT));
        CHECK_CUDA(curandSetPseudoRandomGeneratorSeed(curandGen, 1234ULL));
    }
    ...
    void fillMatrixRandom() {
        ...
        CHECK_CUDA(curandGenerateUniform(curandGen, d_matrix, n * n));
